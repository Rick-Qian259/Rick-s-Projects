...
vector<ast> class_vars ;             // class var
vector<ast> class_vars2 ;            // class var2
vector<ast> class_vars3 ;            // class var3

vector<ast> subr_vars ;             // subr var

symbols store_var_table = create_variables() ;
...
ast copy_class(ast t)
{
    ...
    ast subr_decs_copy = copy_subr_decs(subr_decs) ;            // 2nd

    ast var_decs_copy = copy_class_var_decs(var_decs) ;         // 1st

    //create_class_var_decs(vector<ast> vars) ;
    // var_decs_copy = create_class_var_decs(class_vars) ;
...
ast copy_class_var_decs(ast t)
{
    ...
    write_to_traces("size: " + to_string(ndecs)) ;
...
ast copy_var_dec(ast t)
{
    ...
    if (segment == "this")
    {
        st_variable var_myvar1 = lookup_variables(store_var_table, name) ;
        if (var_myvar1.offset == -1)
        {
            write_to_traces(" name: " + name + " segment: " + segment ) ;
            st_variable myvar(name, type, segment, offset) ;
            insert_variables(store_var_table, name, myvar) ;
        }
    }

    st_variable var_myvar = lookup_variables(store_var_table, name) ;
    if (var_myvar.offset != -1)
    {
        var_dec = create_var_dec(var_myvar.name, var_myvar.segment, var_myvar.offset, var_myvar.type) ;
        return var_dec ;
    }
...
ast copy_function(ast t)
{
    ...
    // ast create_subr_body(ast decs,ast body) ;
    // subr_body_copy = create_subr_body(subr_body) ;
...
ast copy_subr_body(ast t)
{
    ...
    ast body_copy = copy_statements(body) ;         // 2nd
    ast decs_copy = copy_var_decs(decs) ;           // 1st

    // ast create_var_decs(vector<ast> vars) ;
    decs_copy = create_var_decs(subr_vars) ;
...
ast copy_var_decs(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        string type = get_var_dec_type(deci) ;
        string segment = get_var_dec_segment(deci) ;
        int offset = get_var_dec_offset(deci) ;

        // ast create_var_dec(string name,string segment,int offset,string type) ;
        // ast copy = create_var_dec(name, segment, offset, type) ;
        if ( deci != copy ) copied = true ;
...
ast copy_while(ast t)
{
    ...
    ast body_copy = nullptr ;
    ast condition_copy = nullptr ;

    condition_copy = copy_expr(condition) ;             // 1st
    body_copy = copy_statements(body) ;                 // 2nd

    // ast create_statements(vector<ast> statements) ;
    // condition_copy = create_expr() ;
    // body_copy = create_statements(statements) ;
...
ast copy_var(ast t)
{
    ...
    ast temp2 = nullptr ;
    // for class variables
    // temp1 = class_vars.push_back( create_var_decs(class_vars2) ) ;
    // class_var.push_back( temp 1) ;
