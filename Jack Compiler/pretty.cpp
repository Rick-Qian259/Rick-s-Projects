...
// after each left brace, there should add four spaces for indentation
void print_indent(int braces)
{
...
int braces = 0 ;                // how many left braces
bool have_var = false ;         // whether it has var
bool whe_stat = false ;         // whether it is a new statement
bool whe_func = false ;         // whether it is a new function
bool whe_static = false ;       // whether it is a static variable
bool whe_field = false ;        // whether it is a field variable
...
bool have_static = false ;          // for static
bool have_field = false ;           // for field
...
void walk_class_var_decs(ast t)
{
    ...
    // first print the static 
    for ( int i = 0 ; i < ndecs ; i++ )
    {
        if ( have_static && i == ndecs - 1 )
        {
            have_var = true ;
        }
        walk_var_dec(get_class_var_decs(t,i)) ;
        if ( need_print && have_static && i == ndecs - 1 )
        {
            write_to_output("\n") ;
        }
    }
    ...
    // second print the field
    for ( int i = 0 ; i < ndecs ; i++ )
    {
        if ( have_field && i == ndecs - 1 )
        {
            have_var = true ;
        }
        walk_var_dec(get_class_var_decs(t,i)) ;
        if ( need_print && have_field && i == ndecs - 1 )
        {
            write_to_output("\n") ;
        }
    }
...
void walk_var_dec(ast t)
{
    ...
    // write_to_output("segment: " + segment + "\n") ;
    ...
    else if ( whe_static && segment == "static" )
    {
        print_indent(braces) ;
        write_to_output("static " + type + " " + name + " ;\n") ;
        have_static = true ;
    }
    else if ( whe_field && segment == "this" )
    {
        print_indent(braces) ;
